...
public class Window {
    ...
    public void setKeyReleaseCallback(KeyReleaseCallback cb) {
        glfwSetKeyCallback(handle, (window, key, scancode, action, mods) -> {
    ...
    public void setInputCallbacks(KeyCallback onKey, KeyReleaseCallback onKeyRelease) {
        glfwSetKeyCallback(handle, (window, key, scancode, action, mods) -> {
            if (action == GLFW_RELEASE) {
                onKeyRelease.onKey(key);
            } else {
                onKey.onKey(key, action);
            }
        });
    }
