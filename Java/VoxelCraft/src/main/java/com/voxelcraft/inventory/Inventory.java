...
public class Inventory {
    ...
    public boolean consumeSlot(int index, int count) {
        if (index < 0 || index >= TOTAL_SLOTS) return false;
        ItemStack slot = slots[index];
        if (slot.type == null || slot.count < count) return false;
        slot.count -= count;
        if (slot.count <= 0) slots[index] = new ItemStack(null, 0);
        notifyChange();
        return true;
    }

    public ItemStack takeOneFromSlot(int index) {
        if (index < 0 || index >= TOTAL_SLOTS) return new ItemStack(null, 0);
        ItemStack slot = slots[index];
        if (slot.type == null || slot.count <= 0) return new ItemStack(null, 0);
        BlockType type = slot.type;
        consumeSlot(index, 1);
